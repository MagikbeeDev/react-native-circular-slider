...
export default class CircularSlider extends PureComponent
{
  ...
  UNSAFE_componentWillMount()
  {
    ...
    this._wakePanResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, { moveX, moveY }) =>
      {
        ...
        onUpdate({ startAngle, angleLength: this.toFixed(newAngleLength, 5) })
      }
    })
  }
  ...
  render()
  {
    const {
      ...
      startIcon, stopIcon, scrollY, scrollX
    } = this.props
    ...
    return (
      <View key={scrollY} style={{ width: containerWidth, height: containerWidth, justifyContent: 'center' }} onLayout={this.onLayout}>
        <Svg
